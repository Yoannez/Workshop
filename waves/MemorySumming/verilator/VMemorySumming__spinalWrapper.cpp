...
public:
    ...
    Wrapper_1(const char * name){
      ...
      waveEnabled = true;
      signalAccess[0] = new CDataSignalAccess( top.io_wr_en );
      signalAccess[1] = new CDataSignalAccess( top.io_wr_addr );
      signalAccess[2] = new SDataSignalAccess( top.io_wr_data );
      signalAccess[3] = new CDataSignalAccess( top.io_sum_start );
      signalAccess[4] = new CDataSignalAccess( top.io_sum_done );
      signalAccess[5] = new SDataSignalAccess( top.io_sum_value );
      signalAccess[6] = new CDataSignalAccess( top.io_wr_clk );
      signalAccess[7] = new CDataSignalAccess( top.io_sum_clk );
      signalAccess[8] = new CDataSignalAccess( top.io_sum_reset );

      #ifdef TRACE
      Verilated::traceEverOn(true);
      top.trace(&tfp, 99);
      tfp.open((std::string("/home/qichao/SpinalWorkshop/waves/MemorySumming/") + name + ".vcd").c_str());
      #endif
      this->name = name;
    }
    ...
    virtual ~Wrapper_1(){
      ...
      #endif
      #ifdef COVERAGE
      VerilatedCov::write((("/home/qichao/SpinalWorkshop/waves/MemorySumming/") + name + ".dat").c_str());
      #endif
    }
