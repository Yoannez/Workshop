...
case class Apb3Decoder(apbConfig : Apb3Config, outputsMapping : Seq[Mapping]) extends Component{
  ...
  val addressHit = outputsMapping.map(mapping => mapping.hit(io.input.PADDR))
  // Get the index of 1 in a onehot code
  val addressHitIndex = OHToUInt(addressHit)
  for(i <- 0 until outputsMapping.length){
    io.outputs(i).PSEL.lsb := addressHit(i) && io.input.PSEL.lsb
  }

  io.input.PREADY := io.outputs(addressHitIndex).PREADY
  io.input.PRDATA := io.outputs(addressHitIndex).PRDATA
  if(apbConfig.useSlaveError) io.input.PSLVERROR := io.outputs(addressHitIndex).PSLVERROR
}
